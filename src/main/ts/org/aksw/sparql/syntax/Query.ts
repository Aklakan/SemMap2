module org.aksw.sparql.syntax {
	...
	export interface Expr {		
		copy(args: Expr[]) : Expr;
		getArgs(): Expr[];
		copySubstitute(fnNodeMap: any): Expr;
	}

	export class ExprBase1
		implements Expr
	{
		private arg: Expr;
		...
		extends ExprBase2
	{
		...
		copySubstitute(fnNodeMap: any): E_LogicalAnd {
			return new E_LogicalAnd(this.left.copySubstitute(fnNodeMap), this.right.copySubstitute(fnNodeMap));
		}

		copy(args: Expr): E_LogicalAnd {
			return Utils.newBinaryExpr(E_LogicalAnd, args);
		}
		...
		extends ExprBase2
	{
		...
		copySubstitute(fnNodeMap: any): E_LogicalOr {
			return new E_LogicalOr(this.left.copySubstitute(fnNodeMap), this.right.copySubstitute(fnNodeMap));
		}

		copy(args: Expr): E_LogicalOr {
			return Utils.newBinaryExpr(E_LogicalOr, args);
		}
		...
		extends ExprBase1
	{
		...
		copySubstitute(fnNodeMap: any): E_LogicalNot {
			return new E_LogicalNot(this.arg.copySubstitute(fnNodeMap));
		}

		copy(args: Expr): E_LogicalNot {
			return Utils.newUnaryExpr(E_LogicalNot, args);
		}
	...
	export class Utils {
		public static varPattern: string = /\?(\w+)/g;
		public static prefixPattern: string = /\s+(\w+):\w+\s+/g;

		static orify(exprs : Expr[]) : Expr {
			var result = Utils.opify(exprs, E_LogicalOr);
			return result;
		}

		static andify(exprs) : Expr {
			var result = Utils.opify(exprs, E_LogicalAnd);
			return result;
		}
	
		static opify = function(exprs : Expr[], fnCtor : any) : Expr {
			var open = exprs;
			var next = [];
		...
		static uniqTriple(triples : Triple) : Triple[] {
		...
		static extractPrefixes(str) {
			return ns.extractAll(ns.prefixPattern, str, 1);	
		}

		static extractAll(pattern, str, index) {
	...
	// FIXME Possibly rename to RdfNode
	class Node {
		...
		constructor(type: string, value: string, language: string, datatype: string) {
			this.type = type;
			this.value = value;
		...
		static fromJson(talisJson) {			
			...
			var result = new Node(
					type,
					talisJson.value,
					talisJson.lang ? talisJson.lang : "",
					talisJson.datatype? talisJson.datatype : ""
			);
